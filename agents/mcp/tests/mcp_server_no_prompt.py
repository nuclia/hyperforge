...
async def broken_list_prompts(*args, **kwargs):
    raise Exception("MCP Exception: list_prompts failed")


@asynccontextmanager
async def run_mcp_server_no_prompt():
    import uvicorn

    starlette_app = mcp.streamable_http_app()
    # override the list_prompts handler to simulate a failure
    mcp._mcp_server.request_handlers[ListPromptsRequest] = broken_list_prompts
    port = free_port()
