    ...
    async def process_tool(
        ...
        if tool_result.isError:
            ...
            context.chunks.append(
                Chunk(
                    chunk_id=f"mcp_{self.config.id}_{tool_name}_error_{uuid4().hex[:8]}",
        ...
        for block in tool_result.content:
            if isinstance(block, types.TextContent):
                context.chunks.append(
                    Chunk(
                        chunk_id=f"mcp_{self.config.id}_{tool_name}_{uuid4().hex[:8]}",
                ...
                async with self._open_session(manager, memory) as session:
                    ...
                    for message in prompt_result.messages:
                        if isinstance(message.content, types.TextContent):
                            context.chunks.append(
                                Chunk(
                                    chunk_id=f"mcp_prompt_{self.config.id}_{prompt_name}_{uuid4().hex[:8]}",
