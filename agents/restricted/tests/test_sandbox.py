...
@asynccontextmanager
async def configured_sandbox(monkeypatch, **environment: str):
...
@pytest.fixture
async def sandbox_conn(monkeypatch, long_timeout):
    with tempfile.TemporaryDirectory() as path:
        socket = f"{path}/sandbox.sock"
        monkeypatch.setenv("SANDBOX_SOCKET", socket)
        monkeypatch.setenv("SANDBOX_VERIFY", "false")
...
@pytest.fixture
async def tokenless_sandbox_conn(monkeypatch, long_timeout):
    with tempfile.TemporaryDirectory() as path:
        socket = f"{path}/sandbox.sock"
        monkeypatch.setenv("SANDBOX_SOCKET", socket)
        monkeypatch.setenv("SANDBOX_VERIFY", "false")
...
async def test_sandbox_enforces_server_runtime_ceiling(monkeypatch):
    request = WorkerExecutionRequest(
        code="while True: pass",
        question="Q?",
        local_vars={},
    ...
    async with configured_sandbox(
        monkeypatch,
        SANDBOX_CALLBACK_WAIT_SECONDS="15",
        SANDBOX_MAX_SESSION_RUNTIME_SECONDS="0.1",
    ) as socket:
        rx_raw, tx_raw = await asyncio.open_unix_connection(socket)
        rx = sandbox.SandboxReader(rx_raw)
        tx = sandbox.SandboxWriter(tx_raw)
        try:
            await tx.write_message(SandboxMessage.Run(request, token="test-token"))
...
async def test_sandbox_rejects_concurrent_session(monkeypatch):
    request = WorkerExecutionRequest(
        code="foo()",
        question="Q?",
        local_vars={},
    ...
    async with configured_sandbox(
        ...
        first_rx_raw, first_tx_raw = await asyncio.open_unix_connection(socket)
        first_rx = sandbox.SandboxReader(first_rx_raw)
        first_tx = sandbox.SandboxWriter(first_tx_raw)
        try:
            await first_tx.write_message(
                SandboxMessage.Run(request, token="test-token")
            )
            assert isinstance(await first_rx.read_message(), SandboxMessage.Request)

            second_rx_raw, second_tx_raw = await asyncio.open_unix_connection(socket)
            second_rx = sandbox.SandboxReader(second_rx_raw)
            second_tx = sandbox.SandboxWriter(second_tx_raw)
            try:
                await second_tx.write_message(
                    SandboxMessage.Run(request, token="test-token")
                )
